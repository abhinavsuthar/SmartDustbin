...
const addNewDustbin = (req, res, dbs) => {
    ...
    if (uRole !== 'admin' && uRole !== 'editor') {
        return res.status(200).send({
            success: false,
            message: 'Only admin/editor can add new database. Try to login with a different user.'
        })
    }
    ...
    if (body._id) body._id = ObjectId(body._id)
...
const updateDustbinLevel = (req, res, dbs) => {
    ...
    const value = {
        $set: {status: {timestamp: timestamp, level: level}},
        $push: {history: {timestamp: timestamp, level: level}}
    }
    const collection = dbs.production.collection('dustbin')
